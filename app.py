...
def run_dcf(symbol, growth, terminal, margin_of_safety, wacc_override=None):
    ...
    result = {
        ...
        "debt_weight":        round(debt_w * 100, 2),
        "fcf_note":           fcf_note,
        "fcf":                round(fcf / 1e9, 2),
        "fcf_history":        fcf_history,
        "fcf_years":          [str(y)[:10] for y in fcf_years],
        "fcf_cagr":           round(fcf_cagr, 2) if fcf_cagr else None,
...
def result_to_db_entry(r):
    return {
        ...
        "P/E":              round(r.get("pe"), 2) if r.get("pe") else None,
        "Forward P/E":      round(r.get("forward_pe"), 2) if r.get("forward_pe") else None,
        "P/B":              round(r.get("pb"), 2) if r.get("pb") else None,
        "EV/EBITDA":        round(r.get("ev_ebitda"), 2) if r.get("ev_ebitda") else None,
        "FCF (Bn)":         r["fcf"],
